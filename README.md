...
### 2. AI 诊断配置
本项目支持多种 AI 提供商（DeepSeek、Google Gemini、OpenAI、Ollama）进行智能分析。配置方式：

**推荐方式：图形界面配置**
1. 启动程序后，点击主界面右上角的 **"配置 API"** 按钮
2. 在配置对话框中选择模型、填写 API Key 和端点地址
3. 点击"保存且测试"完成配置
4. 配置会自动保存，下次启动无需重新输入

**高级方式：环境变量配置**
- 查阅根目录下的 `AI_CONFIG.md` 了解详细配置方法
- 通过环境变量设置 API Key 和对应的模型

**配置验证**
- 启动程序后，状态栏会显示 `AI 状态：已连接{提供商名称}`（绿色），表示配置成功
- 如果显示 `未连接`（灰色），请检查配置或点击"配置 API"按钮进行设置
