...
**错误处理增强功能**:
当查询失败时，界面会自动显示以下内容：
- **错误信息**: 显示详细的错误原因（如网络超时、API 错误等）
- **API 地址**: 显示完整的 API 请求 URL（格式：`https://api.vore.top/api/IPdata?ip={IP}`）
- **复制按钮**: 一键复制 API URL 到剪贴板，方便用户在浏览器中手动验证
- **重新抓取按钮**: 点击后清除该 IP 的缓存并重新查询，支持失败后手动重试
...
**查询失败时的操作**:
- 如果查询失败，界面会显示错误信息和 API 地址
- 可以点击"复制"按钮复制 API URL，然后在浏览器中手动访问验证
- 可以点击"重新抓取"按钮清除缓存并重新查询该 IP
...
4. **缓存管理**: 
   - 当前缓存在程序运行期间一直保留。如果需要定期清理缓存，可以添加定时任务或 LRU 缓存机制
   - 支持使用 `removeFromCache(String ip)` 方法清除单个 IP 的缓存，强制重新查询
   - UI 中的"重新抓取"按钮会自动调用此方法，清除缓存后重新查询

5. **重试机制**: 
   - 服务支持自动重试（最多2次），提高查询成功率
   - 查询失败后，用户可以通过"重新抓取"按钮手动触发重新查询，无需重启程序
...
4. **UI 测试**:
   ...
   - 测试异步查询时的 UI 响应
   - 测试错误信息的显示
   - 测试查询失败时 API URL 的显示和复制功能
   - 测试"重新抓取"按钮的功能（清除缓存并重新查询）
...
- ✅ 异常处理（不影响主流程）
- ✅ 异步查询支持（不阻塞 UI）
- ✅ 错误处理增强（显示 API URL、支持复制、支持重新抓取）
- ✅ 手动重试功能（清除单个 IP 缓存并重新查询）
